...
/**
 * Gateway root. When signed in, shows a small dashboard (who you are + how to
 * connect an MCP client + sign out); otherwise the marketing landing. In the
 * quantakrypto brand.
 */
export default async function Home() {
  const base = process.env.BETTER_AUTH_URL ?? "https://mcp.quantakrypto.com";
  const command = `claude mcp add --transport http quantakrypto ${base}/mcp`;
        ...
        {session?.user ? (
          <>
            ...
            <p className="mt-4 text-lg text-muted">
              Signed in as <span className="text-ink">{session.user.email}</span>. Add the hosted
              MCP to your AI coding agent:
            </p>
            ...
            <p className="mt-4 text-sm leading-relaxed text-muted">
              Or add <code className="font-mono text-ink">{base}/mcp</code> as an HTTP MCP server in
              Cursor or VS Code. When your client runs the OAuth flow you&rsquo;ll approve access in
              one click — you&rsquo;re already signed in — and it stores a 30-day token.
            </p>
          ...
          <>
            ...
            <p className="mt-5 max-w-xl text-pretty text-lg leading-relaxed text-muted">
              An OAuth-gated Model Context Protocol endpoint for AI coding agents. Sign in to obtain
              a 30-day token, then point your client at{" "}
